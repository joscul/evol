...
public:
    ...
    void run_generation(int population_size, std::function<double(const gp_tree<state> &)>  utility_function) {
        ...
        std::cout << gp_tree<int>::tree_to_string(m_last_generation[permutation[0]]);
        
        const int percentile_10 = permutation.size() * 0.1;
        for (int i = 0; i < percentile_10; i++) {
            auto new_individual = random_mutation(m_last_generation[permutation[i]]);
            next_generation.emplace_back(std::move(m_last_generation[permutation[i]]));
    ...
    gp_tree<state> random_mutation(const gp_tree<state> &tree) {
        return gp_tree<state>::make_random_tree(m_num_params, m_max_depth, m_max_width, m_nodes);
    }
